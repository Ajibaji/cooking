...
router.get('/documentlist', function(req, res) {
	...
	apiActions.getDocuments(req.db.get(config.recipeDataBase), userId, function(data) {
		res.json(data);
	});
});

router.get('/allDocuments', function(req, res) {
	apiActions.getAllDouments(req.db.get(config.recipeDataBase), function(data) {
		res.json(data);
	});
});
...
router.get('/documentlist/:query', function(req, res) {
	...
	apiActions.getDocument(req.db.get(config.recipeDataBase), userId, query, function (data) {
		res.json(data);
	});
});

router.post('/adddocument', function(req, res) {
	var collection = req.db.get(config.recipeDataBase);
