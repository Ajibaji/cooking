...
router.get('/recipe/:query', function(req, res) {
	...
	apiActions.getUserInfo(collection, userId, function(user) {
		...
		apiActions.getDocumentByTitle(req.db.get('recipes'), 'j:"' + req.cookies.cookinguser + '"', query, function(docs) {
			res.render('recipe', {
				title: 'Recipe',
...
router.post('/login', function(req, res) {
	...
	apiActions.getUserID(collection, req.body.username, function(id) {
		if(id === undefined) {
			...
			apiActions.matchPassword(collection, req.body.username, req.body.password, function(pass) {
				if(!pass) {
					res.status(418).send();
				} else {
...
router.post('/register', function(req, res) {
	...
	apiActions.getUserID(collection, req.body.username, function(id) {
		if(id !== undefined) {
			...
			apiActions.registerUser(collection, userDetails, function(doc) {
				res.cookie('cookinguser', doc._id);
				res.status(200).send();
			});
		}
	});
